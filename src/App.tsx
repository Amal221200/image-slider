...
function App() {
  ...
  useEffect(() => {
    if (open) {
      ...
      const newIndex = imageIndex ?? selectedId.current
      document.querySelector<HTMLButtonElement>(`#image-${newIndex}`)?.focus()
      selectedId.current = newIndex
      return
    }
    ...
    const handleKeyNavigation = (e: KeyboardEvent) => {
      ...
      if (e.key === 'ArrowLeft') {
        ...
        const newId = selectedId.current === (images.length -1) ? 0 : selectedId.current + 1
        document.querySelector<HTMLButtonElement>(`#image-${newId}`)?.focus()
        selectedId.current = newId
      } else if (e.key === 'ArrowUp') {
        const potentialId = selectedId.current === -1 ? images.length - 1 : selectedId.current - cols
        const newId = potentialId > 0 ? potentialId : 0
        document.querySelector<HTMLButtonElement>(`#image-${newId}`)?.focus()
        selectedId.current = newId
      }
      else if (e.key === 'ArrowDown') {
        const potentialId = selectedId.current === -1 ? 0 : selectedId.current + cols
        const newId = potentialId < images.length - 1 ? potentialId : images.length - 1
        document.querySelector<HTMLButtonElement>(`#image-${newId}`)?.focus()
        selectedId.current = newId
      }
      else if (e.key === 'Escape') {
        document.querySelector<HTMLButtonElement>(`#image-${selectedId.current}`)?.blur()
    ...
    <main className='mx-auto grid max-w-7xl grid-cols-2 content-center justify-center gap-2 p-4 sm:grid-cols-3 sm:px-4 md:h-screen md:grid-cols-4 lg:grid-cols-5'>
      {
        images.map((image, idx) => (
          <article key={image} className="group/container relative cursor-pointer overflow-hidden">
            <button id={`image-${idx}`} onBlur={() => selectedId.current = -1} onClick={() => onOpen(idx)} className="group/button h-full w-full focus:ring-white focus:ring-offset-2 focus-visible:ring-white focus-visible:ring-offset-2">
